...
class Vnet(object):
    ...
    def get_vnet(self):
        ...
        stage = '1'
        in1 = self.identity_transformed(inputs, base, stage)

        out1 = self.convolutional_block(inputs, base, 5, stage=stage, block='1', activation=False, padding = 'same')
        add1 = Add(name='add1')([out1, in1])
        act1 = PReLU(name='act1_1')(add1)

        out1 = self.convolutional_block(act1, base, 2, stage=stage, block='2', padding = 'same', strides=(2, 2))

        # ----------------- Layer 2-------------------------------
        
        stage = '2'
        in2 = self.identity_transformed(out1, base * 2, stage)
        out2 = self.convolutional_block(out1, base * 2, 5, stage=stage, block='1', padding='same')
        out2 = self.convolutional_block(out2, base * 2, 5, stage=stage, block='2', activation=False, padding='same')
        add2 = Add(name='add2')([out2, in2])
        act2 = PReLU(name='act2_2')(add2)

        drop2= Dropout(keep_prob, name='drop2')(act2)
        out2 = self.convolutional_block(drop2, base * 2, 2, stage=stage, block='3', padding = 'same', strides=(2, 2))

        # ----------------- Layer 3 -------------------------------
        
        stage = '3'
        in3 = self.identity_transformed(out2, base * 4, stage)
        out3 = self.convolutional_block(out2, base * 4, 5, stage=stage, block='1', padding='same')
        out3 = self.convolutional_block(out3, base * 4, 5, stage=stage, block='2', padding='same')
        out3 = self.convolutional_block(out3, base * 4, 5, stage=stage, block='3', activation=False, padding='same')
        add3 = Add(name='add3')([out3, in3])
        act3 = PReLU(name='act3_3')(add3)
        
        drop3 = Dropout(keep_prob, name='drop3')(act3)
        out3 = self.convolutional_block(drop3, base * 2, 2, stage=stage, block='5', padding = 'same', strides=(2, 2))

        # ----------------- Layer 4 -------------------------------
        
        stage = '4'
        in4 = self.identity_transformed(out3, base * 8, stage)
        out4 = self.convolutional_block(out3, base * 8, 5, stage=stage, block='1', padding='same')
        out4 = self.convolutional_block(out4, base * 8, 5, stage=stage, block='2', padding='same')
        out4 = self.convolutional_block(out4, base * 8, 5, stage=stage, block='3', activation=False, padding='same')
        add4 = Add(name='add4')([out4, in4])
        act4 = PReLU(name='act4_3')(add4)            

        drop4 = Dropout(keep_prob, name='drop4')(act4)
        out4 = self.convolutional_block(drop4, base * 2, 2, stage=stage, block='4', padding = 'same', strides=(2, 2))

        # ----------------- Layer 5 -------------------------------
        
        stage = '5'
        in5 = self.identity_transformed(out4, base * 16, stage)
        out5 = self.convolutional_block(out4, base * 16, 5, stage=stage, block='1', padding='same')
        out5 = self.convolutional_block(out5, base * 16, 5, stage=stage, block='2', padding='same')
        out5 = self.convolutional_block(out5, base * 16, 5, stage=stage, block='3', activation=False, padding='same')

        add5 = Add(name='add5')([out5, in5])
        act5 = PReLU(name='act5_3')(add5)            

        drop5 = Dropout(keep_prob, name='drop5')(act5)

        # ----------------- Layer 6 -------------------------------
        
        stage = '6'
        out6 = self.deconvolutional_block(drop5, base * 8, 2, stage=stage, block='1')
        in6 = self.identity_transformed(out6, base * 16, stage)
        out6 = concatenate([drop4, out6], axis = 3, name='concat' + stage)
        out6 = self.convolutional_block(out6, base * 16, 5, stage=stage, block='2', padding='same')
        out6 = self.convolutional_block(out6, base * 16, 5, stage=stage, block='3', padding='same')
        out6 = self.convolutional_block(out6, base * 16, 5, stage=stage, block='4', activation=False, padding='same')
        ...
        stage = '7'
        out7 = self.deconvolutional_block(act6, base * 4, 2, stage=stage, block='1')
        in7 = self.identity_transformed(out7, base * 8, stage)
        out7 = concatenate([drop3, out7], axis = 3, name='concat' + stage)
        out7 = self.convolutional_block(out7, base * 8, 5, stage=stage, block='2', padding='same')
        out7 = self.convolutional_block(out7, base * 8, 5, stage=stage, block='3', padding='same')
        out7 = self.convolutional_block(out7, base * 8, 5, stage=stage, block='4', activation=False, padding='same')
        ...
        stage = '8'
        out8 = self.deconvolutional_block(act7, base * 2, 2, stage=stage, block='1')
        in8 = self.identity_transformed(out8, base * 4, stage)
        out8 = concatenate([drop2, out8], axis = 3, name='concat' + stage)
        out8 = self.convolutional_block(out8, base * 4, 5, stage=stage, block='2', padding='same')
        out8 = self.convolutional_block(out8, base * 4, 5, stage=stage, block='3', activation=False, padding='same')
        ...
        stage = '9'
        out9 = self.deconvolutional_block(act8, base, 2, stage=stage, block='1')
        in9 = self.identity_transformed(out9, base * 2, stage)
        out9 = concatenate([act1, out9], axis = 3, name='concat' + stage)
        out9 = self.convolutional_block(out9, base * 2, 5, stage=stage, block='2', activation=False, padding='same')
